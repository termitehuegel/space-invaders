...
bool BaseFragment::detectCollision(std::vector<Projectile*>* enemy_projectiles, std::vector<Projectile*>* player_projectiles) {
    ...
    for (std::vector<Projectile*>::iterator iter = player_projectiles->begin(); iter != player_projectiles->end(); iter++) {
        if ((*iter)->collidesWith(sprite.getGlobalBounds())) {
            delete *iter;
